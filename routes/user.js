...
exports.getData = function(req,res){
	...
	exports.getDataYearly = function(req,res){
		// These two variables come from the form on
		// the views/login.hbs page
		console.log("Inside yearly  User.js");
		var json_responses ={};

		mongo.connect(mongoURL, function(){
			console.log('Connected to mongo at: ' + mongoURL);
			var coll = mongo.collection('schoolsYearly');
			coll.find({"array.CURROPER":{$gt:0}},{"array.INSTNM":1, "array.TUITIONFEE_IN":1, "array.C150_4":1, "array.schools.earn":1}).toArray(function(err, user){
				if (user) {
					console.log(user.length);
					json_responses.statusCode = 200;
					json_responses.schools=user;
